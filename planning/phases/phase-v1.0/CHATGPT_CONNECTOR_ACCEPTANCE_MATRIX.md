...
| CAV-012 | MCP read tool | Read-only repo status tool succeeds | Connector connected; configured root points at intended repo; `files.read` granted | Ask ChatGPT to call `repo_toolbox.status` and `git_toolbox.readiness_summary` | Safe read-only status/readiness toolbox actions return structured output and are not blocked by ChatGPT safety layer | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR, REPO_IDENTITY_MISMATCH | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0204, WC-V1-0802 | NOT_RUN | These toolbox actions avoid caller-supplied absolute roots and raw git output. |
...
| CAV-016 | patch workflow | Patch proposal flow succeeds | Connector connected; `files.write` granted if the current tool contract requires it; selected workspace is Git-backed | Ask ChatGPT to propose a small patch for an allowed text file without applying it, then repeat against an `artifact_only` workspace | ChatGPT can propose a patch without direct mutation only in a Git-backed workspace; `artifact_only` receives structured policy denial | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR, SCOPE_MISSING, WORKSPACE_POLICY_DENIED, GIT_REQUIRED | P0_BLOCKER | Manual | Operator | WC-V1-0202, WC-V1-0202A, WC-V1-0203 | NOT_RUN | Verify no file is changed by proposal creation. |
| CAV-017 | patch workflow | Approved patch application is gated | Patch proposal from CAV-016 exists in a Git-backed workspace; local mode and approval state varied intentionally | Attempt patch application without matching proposal or mode, then with valid gate in a controlled Git-backed test path; also attempt against non-Git and `artifact_only` workspaces | Patch application requires a matching proposal, appropriate mode/approval state, and a confirmed Git-backed workspace regardless of legacy `requireGitRoot` | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_DENIED_BY_APP_POLICY, WRITE_MODE_BLOCKED, WORKSPACE_POLICY_DENIED, GIT_REQUIRED, TOOL_CALL_APP_ERROR | P0_BLOCKER | Manual | Operator | WC-V1-0202, WC-V1-0202A, WC-V1-0203 | NOT_RUN | The denied case is as important as the allowed case. |
...
| CAV-021 | source control | Git status/readiness workflow is safety-layer compatible | Connector connected; repo identity verified; `files.read` granted | Ask ChatGPT for safe repo status/readiness using `repo_toolbox.status` and `git_toolbox.readiness_summary` | ChatGPT can request safe repo status/readiness without platform safety false positives | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, REPO_IDENTITY_MISMATCH, TOOL_CALL_APP_ERROR | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0301 | NOT_RUN | This validates toolbox naming/schema safety, not arbitrary shell. |
...
| Permission modes/toolsets | `WC-V1-0202`, `WC-V1-0202A` | Required for CAV-014, CAV-015, CAV-016, CAV-017, and CAV-018. WC-V1-0202A adds per-workspace `git_required`/`artifact_only` policy, artifact write roots, launcher controls, and safe diagnostics; live ChatGPT validation remains required after package/promotion/reconnect. |
