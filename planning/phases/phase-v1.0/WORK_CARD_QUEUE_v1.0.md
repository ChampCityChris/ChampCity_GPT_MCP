...
- Stable domain toolbox expansion should prefer internal allowlisted actions under `repo_toolbox`, `git_toolbox`, `artifact_toolbox`, `diagnostics_toolbox`, `integration_toolbox`, `browser_toolbox`, and `knowledge_toolbox` instead of adding new top-level MCP tools when possible. After `WC-V1-FIX05`, these seven toolbox tools remain the read-visible public toolbox surface; `workspace_write_attached_image` is a bounded write-scoped top-level exception because ChatGPT attachment file parameters must be top-level inputs. Figma belongs under `integration_toolbox` as governed broker behavior; do not add `figma_toolbox`.
...
- Acceptance criteria: Public ChatGPT-facing read-only toolbox surface remains the seven toolbox tools; no `dev_toolbox`, `branch_toolbox`, direct top-level `integrate_to_dev`, legacy direct tool, `figma_toolbox`, or `run_allowed_script` public exposure is added; dry run mutates nothing; execute refuses blockers and only pushes `dev` after validation passes.
