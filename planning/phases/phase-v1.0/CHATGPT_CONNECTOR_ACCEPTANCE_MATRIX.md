...
| CAV-011 | MCP discovery | ChatGPT `tools/list` succeeds | Connector connected; `files.read` granted | In a new ChatGPT conversation, trigger tool discovery or ask for available ChampCity tools | ChatGPT can list exposed tools; required read-only tools are visible | CHATGPT_TOOL_RESULT, CHATGPT_UI_OBSERVATION, APP_LOG | TOOLS_LIST_FAIL, TOOL_NOT_VISIBLE, TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0103, WC-V1-0104 | NOT_RUN | A namespace visible with zero tools is failure, not partial success. |
| CAV-012 | MCP read tool | Read-only repo status tool succeeds | Connector connected; configured root points at intended repo; `files.read` granted | Ask ChatGPT to run a safe status/readiness tool such as `get_commit_readiness` or `git_status` | Safe read-only status/readiness tool returns structured output and is not blocked by ChatGPT safety layer | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR, REPO_IDENTITY_MISMATCH | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0204, WC-V1-0802 | NOT_RUN | Prefer a tool that avoids arbitrary command wording. |
| CAV-013 | MCP read tool | Read project file/search succeeds | Connector connected; allowed root configured; `files.read` granted | Ask ChatGPT to read or search an allowed non-secret Markdown file | Safe file read/search operation works for allowed roots without exposing blocked files or secrets | CHATGPT_TOOL_RESULT, APP_LOG, REDACTED_DIAGNOSTIC_EXPORT | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR, SECRET_SAFETY_BLOCKER | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0205 | NOT_RUN | Include one denied blocked-file probe separately under CAV-019. |
...
| CAV-021 | source control | Git status/readiness workflow is safety-layer compatible | Connector connected; repo identity verified; `files.read` granted | Ask ChatGPT for safe repo status/readiness using purpose-built tools | ChatGPT can request safe repo status/readiness without platform safety false positives | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, REPO_IDENTITY_MISMATCH, TOOL_CALL_APP_ERROR | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0301 | NOT_RUN | This validates tool naming/schema safety, not arbitrary shell. |
| CAV-022 | source control | Operator-free git workflow is defined | Source-control workflow design exists or is being validated | Validate required later checks for pull, stage, commit, push, tag, and release workflows | Matrix identifies required later checks for pull, stage, commit, push, tag, and release workflows | DOCTOR_OUTPUT, GITHUB_OR_RELEASE_EVIDENCE, REDACTED_DIAGNOSTIC_EXPORT | GITHUB_AUTH_BLOCKED, SECRET_SAFETY_BLOCKER, EVIDENCE_INCOMPLETE | P0_BLOCKER | Future | Architect | WC-V1-0301, WC-V1-0302 | NOT_RUN | P0 for v1.0 final; implementation belongs to later Work Cards. |
| CAV-023 | release automation | Release baseline/status workflow is safety-layer compatible | Connector connected; release/tag inspection workflow available; no release mutation authorized | Ask ChatGPT to inspect tag, release, artifact, and validation evidence status safely | ChatGPT can safely inspect tag/release/artifact status without arbitrary shell | CHATGPT_TOOL_RESULT, GITHUB_OR_RELEASE_EVIDENCE, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, RELEASE_ARTIFACT_MISSING, TOOL_CALL_APP_ERROR | P0_BLOCKER | Manual | Operator | WC-V1-0302, WC-V1-0303 | NOT_RUN | Must not publish, edit, delete, tag, or upload during this check. |
...
| --- | --- | --- |
| Safety-layer false positives | `WC-V1-0102` | Required for CAV-011, CAV-012, CAV-013, CAV-021, and CAV-030. |
