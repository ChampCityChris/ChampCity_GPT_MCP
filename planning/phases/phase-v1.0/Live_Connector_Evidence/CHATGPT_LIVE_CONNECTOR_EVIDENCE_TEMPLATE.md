...
| CAV-014 | `artifact_toolbox.save_architect_interview_output` when current mode, scope, and canonical evidence allow it |  | Purpose-built Architect Interview save succeeds through the existing artifact toolbox action without adding a new top-level tool. |  | NOT_RUN |  |  | Use only after the Operator has a completed Project Architect Interview body. |
