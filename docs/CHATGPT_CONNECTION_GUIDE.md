...
For generic Markdown artifact creation, ask ChatGPT to call `artifact_toolbox` with `action: "create_markdown_artifact"`, a configured `workspaceId`, and `params` containing exactly `relativePath`, `content`, and optional `overwrite`. The action writes opaque Markdown bytes to a safe repository-relative `.md` path and returns only generic persistence status.
...
In HTTP mode, `/mcp` requires `Authorization: Bearer <access_token>`. Public read-only `tools/list` exposes `repo_toolbox`, `git_toolbox`, `artifact_toolbox`, `diagnostics_toolbox`, `integration_toolbox`, `browser_toolbox`, and `knowledge_toolbox`. With `files.write` and an enabled local write mode, `tools/list` also exposes `workspace_write_attached_image`, a create-only ChatGPT raster-image attachment importer. `files.read` covers `tools/list` and the toolbox tools. `files.write` is enforced for `workspace_write_attached_image` and inside write-capable toolbox actions, including Markdown/JSON artifact writes, patch proposal/application, integration handoff writing, and git mutating actions. Markdown, JSON artifact, and attached-image writes require mode `docs`, `patch`, or `elevated` and do not require `approvalToken`. Patch application requires mode `patch` or `elevated` and an exact matching non-expired pending proposal. Patch application does not accept an elevated approval token or depend on a ChatGPT approval control. `run_allowed_script` is not exposed publicly. Unauthenticated localhost mode requires explicit opt-in with `CHAMPCITY_GPT_ALLOW_UNAUTH_LOCAL_HTTP=true` and must not be used behind a tunnel. A Cloudflare Tunnel can expose a localhost-bound service to the public internet, so localhost binding is not a substitute for OAuth.
