...
| CAV-011 | MCP discovery | ChatGPT `tools/list` succeeds | Connector connected; `files.read` granted | In a new ChatGPT conversation, trigger tool discovery or ask for available ChampCity tools | ChatGPT can list exposed tools; required read-only tools are visible, including `get_workspace_status_summary`, `get_change_set_readiness_summary`, `get_release_artifact_summary`, and `get_release_publication_summary` | CHATGPT_TOOL_RESULT, CHATGPT_UI_OBSERVATION, APP_LOG | TOOLS_LIST_FAIL, TOOL_NOT_VISIBLE, TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0103, WC-V1-0104 | NOT_RUN | A namespace visible with zero tools is failure, not partial success. |
