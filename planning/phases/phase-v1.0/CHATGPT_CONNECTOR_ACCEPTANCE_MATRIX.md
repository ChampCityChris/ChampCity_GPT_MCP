...
| CAV-011 | MCP discovery | ChatGPT `tools/list` succeeds | Connector connected; `files.read` granted; `files.write` optional for write-scoped discovery | In a new ChatGPT conversation, trigger tool discovery or ask for available ChampCity tools | With `files.read`, ChatGPT lists the seven toolbox tools: `repo_toolbox`, `git_toolbox`, `artifact_toolbox`, `diagnostics_toolbox`, `integration_toolbox`, `browser_toolbox`, and `knowledge_toolbox`; with `files.write` plus enabled local write mode, ChatGPT also lists `workspace_write_attached_image`; legacy top-level tools and obsolete Figma tools are absent | CHATGPT_TOOL_RESULT, CHATGPT_UI_OBSERVATION, APP_LOG | TOOLS_LIST_FAIL, TOOL_NOT_VISIBLE, UNEXPECTED_LEGACY_TOOL_VISIBLE, TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0103, WC-V1-0104, WC-V1-FIX02, WC-V1-FIX05 | NOT_RUN | A namespace visible with zero tools is failure, not partial success. The internal registered schema baseline remains 31; ChatGPT-visible exposure is the seven toolbox tools plus the bounded image writer only when scope and local mode allow it. Stable toolbox and bounded image-writer visibility still require live ChatGPT validation after package promotion. |
| CAV-012 | MCP read tool | Read-only workspace and Git status tools succeed | Connector connected; configured root points at intended project; `files.read` granted | Ask ChatGPT to call `repo_toolbox.status`, `diagnostics_toolbox.workspace_safety_status`, and `git_toolbox.readiness_summary` when the selected workspace is Git-backed | General workspace status/safety returns structured capability output without requiring Git; Git readiness remains source-control-specific and is not blocked by ChatGPT safety layer | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR, WORKSPACE_ID_MISMATCH | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0202H, WC-V1-0204, WC-V1-0802 | NOT_RUN | These toolbox actions avoid caller-supplied absolute roots and raw git output. General workspace safety is not source-control readiness. |
...
| CAV-013A | MCP bounded read tool | Long text inspect/chunk retry succeeds | Connector connected; `files.read` granted; approved benign long Markdown fixture available | Call `repo_toolbox.inspect_text_file`, then `repo_toolbox.read_text_chunk` with returned cursor until `complete: true`; if a chunk is blocked, retry the same cursor with a smaller `maximumBytes` | ChatGPT receives bounded exact chunks with stable source SHA-256, line/byte ranges, continuation cursors, delivery receipts, and no full-text duplication in structured metadata | CHATGPT_TOOL_RESULT, APP_LOG, REDACTED_DIAGNOSTIC_EXPORT | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR, RESULT_DELIVERY_INCOMPLETE, SOURCE_SHA_MISMATCH | P0_BLOCKER | Manual | Operator | WC-V1-0202G, WC-V1-0104B | NOT_RUN | Bounded delivery reduces false-positive blast radius but does not guarantee host acceptance. Do not encode, redact, or paraphrase source text as a workaround. |
| CAV-013B | MCP bounded artifact read | Markdown artifact read uses bounded projection | Connector connected; `files.read` granted; large benign Markdown artifact exists | Call `artifact_toolbox.inspect_artifact_text`, `artifact_toolbox.read_artifact_text_chunk`, and `latest_artifact(includeContent=true)` for the artifact | Artifact reads return bounded projection metadata and MCP chunk content; `list_artifacts` remains metadata-only; large Markdown is not returned as one JSON field | CHATGPT_TOOL_RESULT, APP_LOG, REDACTED_DIAGNOSTIC_EXPORT | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR, RESULT_DELIVERY_INCOMPLETE, ARTIFACT_NOT_FOUND | P0_BLOCKER | Manual | Operator | WC-V1-0202G | NOT_RUN | Use benign canaries for education, medical administration, cybersecurity governance, financial compliance, and violence-prevention terminology. |
| CAV-014 | MCP docs write | Docs-write artifact creation succeeds when mode/scope allows | Connector connected; OAuth `files.write` granted; local write mode permits docs writes; target path is under configured artifact roots when restricted | Ask ChatGPT to create a safe Markdown planning or report artifact in a Git-backed workspace and in a non-Git planning workspace such as `revisionary` | ChatGPT can create safe Markdown/JSON planning artifacts in approved paths when granted correct mode/scope and file/path/evidence controls pass; Git is not required for artifact persistence | CHATGPT_TOOL_RESULT, APP_LOG, REDACTED_DIAGNOSTIC_EXPORT | SCOPE_MISSING, WRITE_MODE_BLOCKED, WORKSPACE_POLICY_DENIED, TARGET_OUTSIDE_ARTIFACT_ROOTS, TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR | P0_BLOCKER | Manual | Operator | WC-V1-0202, WC-V1-0202A, WC-V1-0202H, WC-V1-0802 | NOT_RUN | The artifact must be safe to commit or review and free of private credentials. Local tests do not replace live ChatGPT validation. |
...
| CAV-016 | patch workflow | Patch proposal flow succeeds | Connector connected; `files.write` granted if the current tool contract requires it; selected workspace has patch capability | Ask ChatGPT to propose a small patch for an allowed text file without applying it, then repeat against non-Git and `artifact_only` workspaces | ChatGPT can propose a patch without direct mutation only when explicit patch capability and Git-backed safeguards pass; unavailable patch/Git capability is localized to the patch action | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR, SCOPE_MISSING, WORKSPACE_POLICY_DENIED, GIT_CAPABILITY_UNAVAILABLE | P0_BLOCKER | Manual | Operator | WC-V1-0202, WC-V1-0202A, WC-V1-0202H, WC-V1-0203 | NOT_RUN | Verify no file is changed by proposal creation. |
| CAV-017 | patch workflow | Approved patch application is gated | Patch proposal from CAV-016 exists in a patch-capable Git-backed workspace; local mode and approval state varied intentionally | Attempt patch application without matching proposal or mode, then with valid gate in a controlled Git-backed test path; also attempt against non-Git and `artifact_only` workspaces | Patch application requires a matching proposal, appropriate mode/approval state, and explicit patch/Git capability; denial does not affect later reads or artifact writes | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_DENIED_BY_APP_POLICY, WRITE_MODE_BLOCKED, WORKSPACE_POLICY_DENIED, GIT_CAPABILITY_UNAVAILABLE, TOOL_CALL_APP_ERROR | P0_BLOCKER | Manual | Operator | WC-V1-0202, WC-V1-0202A, WC-V1-0202H, WC-V1-0203 | NOT_RUN | The denied case is as important as the allowed case. |
...
| CAV-021 | source control | Git status/readiness workflow is safety-layer compatible | Connector connected; Git-backed workspace selected; `files.read` granted | Ask ChatGPT for source-control status/readiness using `git_toolbox.status` and `git_toolbox.readiness_summary`; repeat `git_toolbox.status` against a non-Git planning workspace | Git-backed status/readiness succeeds without platform safety false positives; non-Git returns localized Git capability state and does not affect later workspace reads/writes | CHATGPT_TOOL_RESULT, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, WORKSPACE_ID_MISMATCH, TOOL_CALL_APP_ERROR | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0202H, WC-V1-0301 | NOT_RUN | This validates toolbox naming/schema safety, not arbitrary shell. |
...
| CAV-023 | release automation | Release baseline/status workflow is safety-layer compatible | Connector connected; release/tag inspection workflow available; no release mutation authorized | Ask ChatGPT to inspect local artifact status with `artifact_toolbox.release_artifact_summary` and publication status with `artifact_toolbox.release_publication_summary` | ChatGPT can safely inspect tag/release/artifact status without arbitrary shell | CHATGPT_TOOL_RESULT, GITHUB_OR_RELEASE_EVIDENCE, APP_LOG | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, RELEASE_ARTIFACT_MISSING, TOOL_CALL_APP_ERROR | P0_BLOCKER | Manual | Operator | WC-V1-0102, WC-V1-0302, WC-V1-0303 | NOT_RUN | Must not publish, edit, delete, tag, or upload during this check. |
...
| CAV-031 | evidence/export | Diagnostic evidence export is safe | Diagnostic export feature or manual evidence bundle exists | Generate or assemble diagnostic evidence and inspect redaction | Diagnostic/export artifact redacts tokens, secrets, private local config, OAuth stores, arbitrary absolute paths, unsafe string JSON-RPC IDs, raw content, and unnecessary private paths | REDACTED_DIAGNOSTIC_EXPORT, APP_LOG | SECRET_SAFETY_BLOCKER, EVIDENCE_INCOMPLETE | P1_RC_REQUIRED | Hybrid | Builder | WC-V1-0104, WC-V1-0205 | NOT_RUN | `recent_tool_calls` records malformed receipt evidence, SDK request-ID-bound dispatch correlation, result-attempt telemetry, HTTP finish/close evidence, and per-call scope denials as `APP_AUTHORIZATION_DENIED` without fabricated dispatch stages; unsupported actions and malformed inputs are `APP_CONTRACT_REJECTED`; application path/write/workspace denials remain `APP_POLICY_DENIED`; string JSON-RPC IDs are sanitized/capped before tool-call and discovery diagnostics. A P1 issue may still block RC approval if evidence cannot be trusted. |
...
| CAV-033 | planning artifact discovery | Builder Report discovery facade avoids broad recursive glob false positives | Connector connected; `files.read` granted; target repository has phase-local `Builder_Reports` Markdown files | Ask ChatGPT to find Builder Reports through `artifact_toolbox.builder_report_index`, including a known report such as `WC07` in `ChampCity_AI`, then use `artifact_toolbox.builder_report_summary` for one specific report without passing an absolute local root or broad recursive glob | ChatGPT can discover relevant Builder Reports through a safe facade without platform safety false positives, and the facade returns repository-relative paths with bounded metadata | CHATGPT_TOOL_RESULT, CHATGPT_UI_OBSERVATION, APP_LOG, REDACTED_DIAGNOSTIC_EXPORT | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_NOT_VISIBLE, REPORT_DISCOVERY_UNAVAILABLE, TOOL_CALL_APP_ERROR | P0_BLOCKER | Manual | Operator | WC-V1-0102A, WC-V1-0104 | NOT_RUN | WC-V1-0102A adds deterministic local coverage for the 2026-06-29 false positive where broad legacy `list_project_files` with `planning/phases`, `**/BUILDER_REPORT*.md`, high `maxResults`, and an absolute local root was blocked. Live ChatGPT validation is still required. |
| CAV-034 | workspace routing | Explicit multi-workspace toolbox routing selects the intended project | Connector connected; multiple allowed workspaces configured; `files.read` granted; expected projects have distinguishable metadata and at least one non-Git planning workspace | Ask ChatGPT to call `diagnostics_toolbox` with `action: list_workspaces`, then call `repo_toolbox.status`, `repo_toolbox.read_file`, `diagnostics_toolbox.workspace_safety_status`, `git_toolbox.status`, and `artifact_toolbox.builder_report_summary` with explicit workspace IDs for at least two configured projects | ChatGPT sees safe workspace IDs, including valid non-Git project IDs such as `revisionary`; project-specific calls route to the requested workspace; Git absence is informational unless a Git action was requested; ambiguous `workspaceId: default` fails safely when no explicit default is configured | CHATGPT_TOOL_RESULT, APP_LOG, REDACTED_DIAGNOSTIC_EXPORT | TOOL_CALL_BLOCKED_BY_CHATGPT_SAFETY, TOOL_CALL_APP_ERROR, WORKSPACE_ID_MISMATCH, EVIDENCE_INCOMPLETE | P0_BLOCKER | Manual | Operator | WC-V1-FIX04, WC-V1-0104, WC-V1-0202H | NOT_RUN | Local self-test covers fixture routing, but live ChatGPT validation is still required after package promotion. |
...
| Stable domain toolbox surface | `WC-V1-FIX02`, `WC-V1-FIX05` | Public ChatGPT read-only `tools/list` exposes `repo_toolbox`, `git_toolbox`, `artifact_toolbox`, `diagnostics_toolbox`, `integration_toolbox`, `browser_toolbox`, and `knowledge_toolbox`. `workspace_write_attached_image` is the bounded top-level write-scoped exception for ChatGPT attachment file parameters. Future expansion should prefer internal allowlisted actions over new top-level MCP tools when file-parameter constraints do not require top-level input. Write actions still require `files.write` and local write-mode policy. No `figma_toolbox` is added; Figma belongs under `integration_toolbox` as a broker-not-implemented placeholder until a future scoped broker card. |
...
| Permission modes/toolsets | `WC-V1-0202`, `WC-V1-0202A`, `WC-V1-0202H` | Required for CAV-014, CAV-015, CAV-016, CAV-017, and CAV-018. WC-V1-0202H adds the independent workspace capability model, isolates Git requirements to Git/release/patch capability checks, keeps non-Git project workspaces valid for reads and artifact persistence, and separates service runtime provenance from target project metadata; live ChatGPT validation remains required after package/promotion/reconnect. |
...
Active Work Cards should use `dev` or a generated `feature/WC-V1-xxxx-*` / `feature/WC-V1-FIXxx-*` branch. `main` is reserved for stable release or baseline checkpoints. The public safe MCP branch-preparation path is `git_toolbox.prepare_work_branch`, followed by validation, `git_toolbox.stage_paths`, `git_toolbox.pre_commit_scan`, `git_toolbox.commit_staged`, and `git_toolbox.push_current_branch` for the current `dev` or feature branch. Merge to `main` only at a stable checkpoint.
