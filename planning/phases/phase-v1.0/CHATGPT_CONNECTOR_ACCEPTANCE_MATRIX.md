...
| Stable domain toolbox surface | `WC-V1-FIX02`, `WC-V1-FIX05` | Public ChatGPT `tools/list` exposes only `repo_toolbox`, `git_toolbox`, `artifact_toolbox`, `diagnostics_toolbox`, `integration_toolbox`, `browser_toolbox`, and `knowledge_toolbox`. Future expansion should prefer internal allowlisted actions over new top-level MCP tools. Write actions still require `files.write` and local write-mode policy. No `figma_toolbox` is added; Figma belongs under `integration_toolbox` as a broker-not-implemented placeholder until a future scoped broker card. |
